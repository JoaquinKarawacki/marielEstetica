...
export default function MisTurnosPage() {
  ...
  async function runSearch(q: string) {
    if (q.trim().length < 3) {
      setError("Ingresá tu celular o el código de reserva (ej: MM-7F3K2).");
      return;
    }
    ...
    <div className="bg-noise min-h-screen bg-cream-100 pt-10 pb-24">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <Link
            href="/"
            className="flex items-center gap-1.5 text-sm text-ink-700/55 hover:text-burgundy-600"
          >
        ...
        >
          <Input
            placeholder="099 123 456 o MM-7F3K2"
            value={query}
        ...
        {error && (
          <p className="mx-auto mt-4 max-w-md rounded-xl bg-burgundy-500/10 px-4 py-2.5 text-center text-sm text-burgundy-600">
            {error}
          </p>
        )}
