    ...
    <>
      ...
      >
        ...
        >
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            {LINKS.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="rounded-full px-3.5 py-2 text-[13.5px] font-medium text-ink-700/70 transition-colors hover:bg-burgundy-500/[0.06] hover:text-burgundy-600"
              >
                {link.label}
              </a>
            ))}
          ...
          <button
            onClick={() => setOpen((o) => !o)}
            className="grid h-9 w-9 place-items-center rounded-full text-burgundy-600 lg:hidden"
      ...
      >
        {LINKS.map((link, i) => (
          <a
            ...
            className="font-display text-2xl text-burgundy-600 transition-all duration-300"
            style={{
